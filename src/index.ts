import { afterLoad, cacheUseRatio, getCLS, getFCP, getFID, getFP, getLCP, getNavigationTiming, getResourceFlow } from './core'
import type { ResourceFlowTiming } from './core'
import MetricsStore, { metricsName } from './store'
import type { IMetrics } from './store'
...
export default class WebVitals {
  ...
  initCLS = (): void => {
    ...
    const entryHandler = (entry: LayoutShift) => {
      if (!entry.hadRecentInput) {
        ...
        // 包含在当前会话中。否则，开始一个新会话。
        if (
          sessionValue && entry.startTime - lastSessionEntry.startTime < 1000 && entry.startTime - firstSessionEntry.startTime < 5000
        ) {
  ...
  // 初始化 RF 的获取以及返回
  initResourceCachRatio = (): void => {
    const stopListening = () => {
      const metrics = {
        ratios: cacheUseRatio,
      } as IMetrics
      this.metrics.set(metricsName.RFCUR, metrics)
    }
    // 当页面 pageshow 触发时，中止
    window.addEventListener('pageshow', stopListening, { once: true, capture: true })
  }
